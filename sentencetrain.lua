...
function gradUpdate(mlpin, x, y, criterionin, lr, optm, limit)
	...
	_inner_err=criterionin:forward(pred, _y)
	local gradCriterion=criterionin:backward(pred, _y)
...
function evaDev(mlpin, criterionin)
	...
	for i,devu in ipairs(devd) do
		...
		serr=serr+criterionin:forward(mlpin:forward(_i), _t[-1])

...
function evaDev(mlpin, criterionin, ftm)
	mlpin:evaluate()
	local serr=0
	xlua.progress(0, ndev)
	for i,devu in ipairs(devd) do
		local _i, _t = unpack(devu)
		serr=serr+criterionin:forward(mlpin:forward(_i), ftm:updateOutput(_t))
...
function train()
	...
	require "dep.maskRFill"
	ftm = nn.maskRFill(1, 4, 0.8)
	ftm:training()
	ftm:cuda()
	...
	mindeverrate=evaDev(nnmod,critmod,ftm)
	...
	for tmpi=1,warmcycle do
		for tmpj=1,ieps do
			...
			for i,trainu in ipairs(traind) do
				...
				gradUpdate(nnmod,_i,ftm:updateOutput(_t),critmod,lr,optmethod,memlimit)
	...
	while cntrun do
		...
		for innercycle=1,gtraincycle do
			...
			local erate=sumErr/eaddtrain
			table.insert(crithis,erate)
			local edevrate=evaDev(nnmod,critmod,ftm)
